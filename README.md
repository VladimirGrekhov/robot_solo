...
orb_account.py        — READ-ONLY чтение средств счёта FORTS (для вкладки «Счёт»)
...
run_orb_window.bat    — запуск окна (GUI)
...
На Windows — то же самое через `run_orb_window.bat` (окно) или
`run_orb_backtest.bat` / `run_orb_paper.bat` / `run_orb_live.bat` (консоль).
