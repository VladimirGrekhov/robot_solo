@echo off
chcp 65001 >nul
rem Launcher for orb_robot.py --mode live (REAL ORDERS).
rem Requires live_trading: true in config_orb.yaml AND manual "yes"
rem confirmation at startup (the robot itself will ask).
rem Before the first live run, go through the checklist in ORB_README.md.
rem
...
echo.
echo WARNING: live mode places REAL orders on the exchange.
